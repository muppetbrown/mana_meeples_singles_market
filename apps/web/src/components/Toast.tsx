...
export const ToastProvider = ({
  ...
  const addToast = (message: any, type = 'info', duration = 5000) => {
  ...
  const toast = {
    success: (message: any, duration: any) => addToast(message, 'success', duration),
    error: (message: any, duration: any) => addToast(message, 'error', duration),
    warning: (message: any, duration: any) => addToast(message, 'warning', duration),
    info: (message: any, duration: any) => addToast(message, 'info', duration),
  };
...
const ToastContainer = ({
  ...
  return (
    <div className="fixed top-4 right-4 z-50 space-y-2">
      {toasts.map((toast: any) => <ToastItem key={toast.id} toast={toast} onRemove={removeToast} />)}
    </div>
  );
};
