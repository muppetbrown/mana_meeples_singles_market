...
const UnifiedCardsTab = ({ mode = 'all' }) => {
  ...
  const handleSearchChange = useCallback(async (value: any) => {
  ...
  const getUniqueTreatments = (variations: any) => {
    if (!variations || variations.length === 0) return [];
    return [...new Set(variations.map((v: any) => v.treatment))];
  };

  const getUniqueFinishes = (variations: any) => {
    if (!variations || variations.length === 0) return [];
    return [...new Set(variations.map((v: any) => v.finish))];
  };

  const toggleCard = (key: any) => {
  ...
  const getFinishBadge = (finish: any) => {
    if (finish === 'foil') {
              ...
              {isExpanded && (
                <div className="border-t border-slate-200 bg-slate-50 p-4">
                  <div className="space-y-2">
                    // @ts-expect-error TS(2339): Property 'variations' does not exist on type 'neve... Remove this comment to see the full error message
                    {card.variations.map((variation: any, idx: any) => (
                      <div
                        key={idx}
