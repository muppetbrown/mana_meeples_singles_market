 ...
 */
async function analyzeSetVariations(setId, gameId) {
    ...
    `, [setId, gameId]);
        ...
        cards.forEach((card) => {
            ...
            if (card.frame_effect) {
                const effects = card.frame_effect.split(',').map((e) => e.trim());
 ...
 */
async function analyzeGameVariations(gameId) {
    const client = await db.getClient();
    try {
        await client.query('BEGIN');
 ...
 */
async function getSetVariations(setId) {
    try {
 ...
 */
async function getGameVariations(gameId) {
    try {
        const result = await db.query(`
      SELECT 
        visual_treatments,
