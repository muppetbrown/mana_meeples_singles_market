...
Items:
${items.map(item => `- ${item.name} (${item.quality}) x${item.quantity} - ${currency}${(item.price * item.quantity).toFixed(2)}`).join('\n')}
...
Questions? Contact us at ${process.env.OWNER_EMAIL || 'support@manameeplescards.com'}
  ...
  async sendOrderNotificationToOwner(orderData) {
    if (!this.isConfigured) {
...
Order Items:
${items.map(item => `- ${item.name} (${item.quality}) x${item.quantity} - ${currency}${item.price.toFixed(2)} each = ${currency}${(item.price * item.quantity).toFixed(2)}`).join('\n')}
