...
class PriceService {
  ...
  static async updatePricesFromTCGPlayer(cardIds: any) {
    try {
  ...
  static async syncMTGCardsFromScryfall(setCode: any) {
    try {
  ...
  static async syncPokemonCards(setId: any) {
    try {
  ...
  // Manual price update
  static async updateManualPrice(inventoryId: any, newPrice: any) {
    try {
...
cron.schedule('0 2 * * *', async () => {
  ...
  try {
    ...
    const tcgplayerIds = items.rows.map((row: any) => row.tcgplayer_id);
