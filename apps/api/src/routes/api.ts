...
const createRateLimit = (windowMs: any, max: any, message = 'Too many requests') => expressRateLimit({
  windowMs,
  max,
  message: {
    error: message,
...
const rateLimit = (windowMs: any, max: any, message = 'Too many requests') => createRateLimit(windowMs, max, message);
 ...
 * ]
 */
router.get('/games', normalRateLimit, async (req: any, res: any) => {
  try {
...
router.get('/sets', normalRateLimit, async (req: any, res: any) => {
  try {
...
router.get('/filter-options', normalRateLimit, async (req: any, res: any) => {
  try {
    const { game_id } = req.query;
    ...
    res.json({
      foilTypes: foilTypesQuery.rows.map((row: any) => row.foil_type),
      cardTypes: cardTypesQuery.rows.map((row: any) => row.card_type),
      qualities: qualitiesQuery.rows.map((row: any) => row.quality),
      rarities: raritiesQuery.rows.map((row: any) => row.rarity)
    });
  } catch (error) {
...
router.get('/cards', normalRateLimit, async (req: any, res: any) => {
  try {
    const {
      game_id,
      set_id,
    ...
    if (search) {
      ...
      if (sanitizedSearch.length === 0) {
        ...
        const searchTerms = sanitizedSearch.split(/\s+/).filter((term: any) => term.length > 0);
...
router.get('/admin/csrf-token', adminAuth, generateCSRFToken, (req: any, res: any) => {
  res.json({
    success: true,
...
router.get('/admin/inventory', adminAuth, async (req: any, res: any) => {
  try {
    const {
      search,
      game_id,
    ...
    const cardIds = cardResults.rows.map((card: any) => card.card_id);
    ...
    for (const card of cardResults.rows) {
      ...
      const cardInventory = existingInventory.filter((inv: any) => inv.card_id === card.card_id);
      const cardInventoryMap = new Map();

      // Map existing inventory by quality-foil-language combination
      cardInventory.forEach((inv: any) => {
...
router.get('/admin/inventory-list', adminAuth, async (req: any, res: any) => {
  try {
    const {
      game,
      set,
 ...
 * }
 */
router.post('/orders', validateCSRFToken, async (req: any, res: any) => {
...
router.get('/admin/orders', adminAuth, async (req: any, res: any) => {
  try {
...
router.get('/admin/orders/:id', adminAuth, async (req: any, res: any) => {
  try {
    const { id } = req.params;
...
router.put('/admin/orders/:id/status', adminAuthWithCSRF, async (req: any, res: any) => {
  const client = await db.getClient();
...
router.put('/admin/inventory/:id', adminAuthWithCSRF, async (req: any, res: any) => {
  try {
    const { id } = req.params;
...
router.get('/search/autocomplete', rateLimit(60000, 50), async (req: any, res: any) => {
  try {
    const { q } = req.query;
...
router.get('/filters', async (req: any, res: any) => {
  try {
    ...
    res.json({
      rarities: rarities.rows.map((r: any) => r.rarity).filter(Boolean),
      qualities: qualities.rows.map((q: any) => q.quality).filter(Boolean),
      foilTypes: foilTypes.rows.map((f: any) => f.foil_type).filter(Boolean),
      cardTypes: cardTypes.rows.map((t: any) => t.card_type).filter(Boolean),
      languages: languages.rows.map((l: any) => l.language).filter(Boolean)
    });
  } catch (error) {
...
router.get('/filters/counts', lenientRateLimit, async (req: any, res: any) => {
  try {
    ...
    const formatCounts = (rows: any) => rows.reduce((acc: any, row: any) => {
...
router.get('/cards/:id/stock', async (req: any, res: any) => {
  try {
    const { id } = req.params;
    ...
    const totalStock = result.rows.reduce((sum: any, item: any) => sum + item.stock, 0);
...
router.get('/cards/:id/current-price', async (req: any, res: any) => {
  try {
    const { id } = req.params;
 ...
 */
router.get('/admin/all-cards', adminAuth, async (req: any, res: any) => {
  try {
    const {
      game_id,
      set_id,
    ...
    const cards = Array.from(cardMap.values()).map(card => ({
      ...
      variations: card.variations.sort((a: any, b: any) => {
 ...
 */
router.post('/admin/inventory', adminAuth, async (req: any, res: any) => {
  try {
    const {
      card_id,
      quality,
...
router.post('/admin/bulk-update', adminAuthWithCSRF, async (req: any, res: any) => {
  const client = await db.getClient();
...
router.post('/admin/csv-import', adminAuthWithCSRF, async (req: any, res: any) => {
  const client = await db.getClient();
...
router.get('/currency/detect', async (req: any, res: any) => {
  try {
...
router.get('/cards/sectioned', async (req: any, res: any) => {
  try {
    const {
      game_id,
      search,
...
router.post('/admin/create-foil', adminAuthWithCSRF, async (req: any, res: any) => {
  try {
...
router.post('/admin/bulk-create-foils', adminAuthWithCSRF, async (req: any, res: any) => {
  const client = await db.getClient();
...
router.post('/admin/refresh-prices', adminAuthWithCSRF, async (req: any, res: any) => {
  const client = await db.getClient();
  ...
  try {
    ...
    for (let i = 0; i < mtgCards.rows.length; i += batchSize) {
      ...
      const scryfallIds = batch.map((card: any) => card.scryfall_id);
      ...
      try {
        ...
        const response = await fetch('https://api.scryfall.com/cards/collection', {
          ...
          body: JSON.stringify({
            identifiers: scryfallIds.map((id: any) => ({
              id
            }))
          })
        });
        ...
        for (const scryfallCard of scryfallData.data) {
          const matchingInventoryItems = batch.filter((item: any) => item.scryfall_id === scryfallCard.id);
...
router.post('/admin/bulk-create-variations', adminAuthWithCSRF, async (req: any, res: any) => {
  const client = await db.getClient();
...
router.post('/admin/import-card-data', adminAuthWithCSRF, async (req: any, res: any) => {
  const client = await db.getClient();
...
router.get('/admin/variations/:card_id', adminAuth, async (req: any, res: any) => {
  try {
...
router.delete('/admin/variations/:variation_id', adminAuthWithCSRF, async (req: any, res: any) => {
  const client = await db.getClient();
