...
async function importMTGSet(setCode: string) {
  ...
  for (const card of allCards) {
    try {
      ...
      const specialFoil = card.promo_types?.find((p: any) => SPECIAL_FOILS.includes(p));
      if (specialFoil) variationStats.specialFoils.add(specialFoil);
      
      const relevantFrames = card.frame_effects?.filter((e: any) => !IGNORE_FRAME_EFFECTS.includes(e)
      ) || [];
      relevantFrames.forEach((f: any) => variationStats.frameEffects.add(f));
