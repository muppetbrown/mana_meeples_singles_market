  ...
  const fetchAnalytics = useCallback(async () => {
    ...
    try {
      ...
      const stats = {
        ...
        totalValue: inventory.reduce((sum, item) => sum + (item.stock_quantity * item.price), 0),
        totalStock: inventory.reduce((sum, item) => sum + item.stock_quantity, 0),
        lowStock: inventory.filter(item => item.stock_quantity > 0 && item.stock_quantity <= 3).length,
